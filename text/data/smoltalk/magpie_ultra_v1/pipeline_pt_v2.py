...
from distilabel.models import vLLM, ClientvLLM
...
class AssignTags(Task):
    ...
    def outputs(self) -> List[str]:
        if self.mission == "dificuldade":
            return ["intencao", "conhecimento", "dificuldade"]

        if self.mission == "qualidade":
            return ["explicacao", "qualidade"]

        return ["tag_primaria", "outras_tags"]
...
with Pipeline(name="magpie-ultra-pt-v0.5.5") as pipeline:
    ...
    assign_difficulty = AssignTags(
        mission="dificuldade",
        llm=ClientvLLM(
            base_url="http://10.100.0.111:8000/v1",
            model="openai/gpt-oss-120b",
            tokenizer="openai/gpt-oss-120b",
            structured_output={ # Sem "format" pois o clientvLLM herda do OpenAI client
...
if __name__ == "__main__":
    ...
    distiset.push_to_hub("EdwardSJ151/magpie-ultra-pt-v0.5.5")
